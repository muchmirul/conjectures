"""Build the web version of this topic's article from ARTICLE.md.
    ...
    cd zeta-critical-line && make docs      (or: ../.venv/bin/python build_docs.py)

Writes ../docs/zeta-critical-line/index.html, the polished one-page version:
...
def render_html(blocks) -> str:
    ...
    for kind, payload in blocks:
        if kind == "h1":
            ...
            if "·" in payload:                    # "5 · Four ways to count"
...
def embed_simulations(body: str) -> str:
    ...
    return body.replace(
        "https://muchmirul.github.io/conjectures/zeta-critical-line/play/",
        "play/")
...
<title>Prime Numbers and Two Thirds of the Zeros, from Zero</title>
...
the pictures, and <code>make test</code> checks the computed numbers and toy
experiments used in the guide.</p>
