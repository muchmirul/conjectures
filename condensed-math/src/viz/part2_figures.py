...
def hero():
    ...
    def update(f):
        ...
        ax.set_title("one number, handed down, always adding back up",
        ...
        for lvl in range(stage + 1):
            ...
            ax.text(1.06, -lvl, f"stage {lvl}   total {total:.0f}",
                    transform=ax.get_yaxis_transform(), color=GREEN,
                    fontsize=9.5, va="center", family="monospace",
                    clip_on=False)
        tag.set_text(f"the split leans {bias:.2f} one way     "
                     f"every stage still totals 64")
        return ()
...
def padic_walk():
    ...
    def update(f):
        ...
        ax.set_title("the ordinary reading: the totals run away", color=RED,
                     fontsize=11.5)
        top = sums[terms - 1]
        for j in range(k + 1):
        ...
        axn.set_title("the base-two reading: the totals close in", color=TEAL,
                      fontsize=11.5)
        x0, w = 0.03, 0.94
        for j in range(k + 1):
        ...
        axn.scatter([x0 + w / 2], [0.49], s=70, color=GREEN, zorder=5,
                    edgecolors="none")
        axn.text(0.5, 0.03, "the value the totals reach: minus one",
                 ha="center", color=GREEN, fontsize=10.5)
        tag.set_text(f"terms added {k + 1}     last total {sums[k]}     "
                     f"ordinary distance {abs(sums[k] + 1)}     "
                     f"base-two distance {2.0 ** -(k + 1):.5f}")
        return ()
...
def distance():
    ...
    ax.set_yscale("log")
    ax.set_title("the same running totals, measured two ways", color=INK2,
                 fontsize=12)
    ks = np.arange(1, terms + 1)
    ax.plot(ks, ordinary, "o-", color=RED, lw=2, ms=5,
            label="the ordinary size: doubles every step")
    ax.plot(ks, padic, "o-", color=TEAL, lw=2, ms=5,
            label="the base-two size: halves every step")
    ax.legend(frameon=False, fontsize=9.5, loc="center left")
    ax.set_xticks(ks[::2])
    fig.text(0.5, 0.025, "computed in exact fractions; the value reached is "
                         "minus one",
...
def integral():
    ...
    fig.subplots_adjust(left=0.06, right=0.97, top=0.87, bottom=0.14,
                        hspace=0.42)
    fig.suptitle("the same measurement, read coarse and read fine",
    ...
    total = a
    fig.text(0.5, 0.035, f"total both times:  {total}    "
...
def steps():
    ...
    def update(f):
        ...
        ax.set_title("rebuilding a measurement out of switches", color=INK2,
                     fontsize=12)
        k = min(n, f)
        xs = np.arange(n) / n
        ...
        for j in range(k):
            ax.text(xs[j] + 0.5 / n, target[j] + 0.28, str(target[j]),
                    ...
                    family="monospace")
        tag.set_text(f"switches used {k} of {n}     "
                     f"whole-number coefficients only"
                     + ("     rebuilt exactly" if k == n else ""))
        return ()
...
def basis_size():
    ...
    ax.set_title("the construction finds exactly one switch per box",
                 color=INK2, fontsize=12)
    x = np.arange(len(depths))
    ax.bar(x - 0.19, boxes, width=0.36, color=BASELINE,
           label="boxes at that stage")
    ax.bar(x + 0.19, found, width=0.36, color=TEAL,
           label="switches the construction keeps")
    ...
    fig.text(0.5, 0.03, "run here on finite stages; the theorem is about the "
                        "endless limit",
             ha="center", color=INK2, fontsize=10)
...
def product_sum():
    ...
    for ax, (title_, mask, colour) in zip(
            axes,
            [("a product: every dial may be set, forever",
              np.ones(n, bool), VIOLET),
             ("a sum: only finitely many may be off zero",
              np.arange(n) < 4, TEAL)]):
        ...
        for k in range(n):
            ...
            if mask[k]:
                ...
                ax.text(x, 0.42, "0", ha="center", va="center", color=MUTED,
                        fontsize=8)
        ax.text((n + 0.9) / (n + 1.4), 0.42, "and on", va="center",
...
def finite_reach():
    ...
    def update(f):
        ax.clear()
        style_axes(ax, (0, 1), (0, 1), equal=False)
        ax.set_title("a question scanning along the dials", color=INK2,
                     fontsize=12)
        at = min(n, f)
        for k in range(n):
            x = (k + 0.5) / (n + 1.2)
            ...
            ax.add_patch(plt.Circle((x, 0.68), 0.019,
                                    ...
                                    lw=1.5))
        ax.text((n + 0.7) / (n + 1.2), 0.68, "and on", va="center",
                color=MUTED, fontsize=9.5)
        head = min(at, n) / (n + 1.2)
        stalled = at > reach
        ...
        if stalled:
            ax.text((reach + 0.4) / (n + 1.2), 0.42,
                    "the scan stops here", color=RED, fontsize=10.5)
            ax.text(0.5, 0.2, "past this point the settings are divisible by "
                              "every power of two,",
                    ha="center", color=INK2, fontsize=10.5)
            ax.text(0.5, 0.11, "so the answer would have to be too, and only "
                               "zero is",
                    ha="center", color=RED, fontsize=10.5)
        tag.set_text(f"dials scanned {min(at, reach)}     "
                     + ("the reach is finite, whatever the question"
                        if stalled else "scanning"))
        return ()
...
def extension():
    ...
    def update(f):
        ax.clear()
        style_axes(ax, (0, 1), (0, 1), equal=False)
        ax.set_title("from points, to weightings, in exactly one way",
                     color=INK2, fontsize=12)
        ...
        ax.text(0.68, 0.76, "its points, placed in the group", ha="center",
                color=VIOLET, fontsize=10.5)
        if t > 0.45:
            a = (t - 0.45) / 0.55
            ax.add_patch(plt.Rectangle((0.5, 0.16), 0.44, 0.34, fc="none",
                                       ec=GREEN, lw=2, alpha=a))
            ax.text(0.72, 0.4, "the one extension", ha="center", color=GREEN,
                    fontsize=11, alpha=a)
            ax.text(0.72, 0.29, "every weighting now has a value",
        ...
        tag.set_text("existence gives you the sums     "
                     "uniqueness stops two answers for one sum"
                     if t > 0.95 else "placing the probe's points")
        return ()
...
def solid_or_not():
    ...
    def dials(ax, filled=8):
        ...
        ax.text(0.99, 0.5, "and on", va="center", ha="right", color=MUTED,
                fontsize=8.5)
    ...
    cells = [("the whole numbers", True, lattice,
              "a weighting has a finite total"),
             ("a row of dials", True, dials,
              "integrate one dial at a time"),
             ("the base-p numbers", True, nesting,
              "built to be completed this way"),
             ("power series in one variable", True, series,
              "one dial per power of the variable"),
             ("finitely many dials at a time", False,
              lambda ax: dials(ax, filled=3),
              "a weighting can spread past any bound"),
             ("the ruler", False, ruler,
              "everything in it divides forever")]
    ...
    fig.suptitle("which groups obey the rule, and what each one looks like",
...
def divisible():
    ...
    def update(f):
        ax.clear()
        style_axes(ax, (0, 1), (0, 1), equal=False)
        ax.set_title("divide, and divide again", color=INK2, fontsize=12)
        k = min(n, f // 4)
        r, z = 720.0, 720.0
        ax.text(0.06, 0.94, "the ruler", color=VIOLET, fontsize=11)
        ax.text(0.56, 0.94, "the whole numbers", color=BROWN, fontsize=11)
        left = True
        for j in range(k + 1):
            div = j + 2
            y = 0.84 - j * 0.088
            ...
            if not inside and left:
                ax.text(0.94, y, "left the group", color=RED, fontsize=9.5,
                        ha="right", va="center")
                left = False
            r /= div
            z /= div
        ax.text(0.5, 0.06, "a group where everything divides forever can send "
                           "only zero to a dial",
                ha="center", color=RED, fontsize=10.5)
        tag.set_text(f"divisions made {k}     "
                     f"the ruler survives all of them     "
                     f"the whole numbers do not")
        return ()
...
def no_map():
    ...
    style_axes(ax, (0, 1), (0, 1), equal=False)
    ax.set_title("why the ruler has nowhere to go", color=INK2, fontsize=12)
    ax.add_patch(plt.Rectangle((0.04, 0.52), 0.3, 0.3, fc="#efeafa",
                               ec=VIOLET, lw=1.8))
    ax.text(0.19, 0.67, "the ruler\neverything divides", ha="center",
            va="center", color=VIOLET, fontsize=11, linespacing=1.5)
    ax.add_patch(plt.Rectangle((0.64, 0.52), 0.3, 0.3, fc="#eef4fb",
                               ec=BLUE, lw=1.8))
    ax.text(0.79, 0.67, "one dial\nwhole numbers only", ha="center",
            va="center", color=BLUE, fontsize=11, linespacing=1.5)
    ax.annotate("", (0.62, 0.67), (0.36, 0.67),
                arrowprops=dict(arrowstyle="-|>", color=RED, lw=2))
    ax.text(0.49, 0.73, "any legal map", ha="center", color=RED, fontsize=10)
    ax.text(0.49, 0.59, "sends everything to 0", ha="center", color=RED,
            fontsize=10)
    ax.text(0.5, 0.36, "the image would have to divide forever inside "
                       "the dial,",
            ha="center", color=INK2, fontsize=11)
    ax.text(0.5, 0.27, "and zero is the only whole number that does",
            ha="center", color=INK2, fontsize=11)
    ax.text(0.5, 0.11, "so the ruler reaches none of the building blocks",
...
def tensor_table():
    ...
    pretty = ["base-p", "base-l", "series in T", "series in U", "the ruler"]

    def show(key):
        if key == "0":
            return "nothing"
        base, _, rest = key.partition("[[")
        head = {"Z_p": "base-p", "Z_l": "base-l", "Z": "whole numbers"}[base]
        if not rest:
            return head
        return head + "\nseries in " + rest.rstrip("]").replace(",", " and ")

    fig, ax = plt.subplots(figsize=(8.4, 5.0))
    fig.subplots_adjust(left=0.16, right=0.98, top=0.79, bottom=0.13)
    style_axes(ax, (-0.02, 1.02), (-0.02, 1.02), equal=False)
    ax.set_title("combining two solid groups", color=INK2, fontsize=12,
                 pad=26)
    n = len(names)
    ...
    ax.scatter([0.02], [-0.075], s=16, color=VIOLET, clip_on=False,
               edgecolors="none")
    ax.text(0.045, -0.075, "the five entries the lectures state outright; "
                           "the rest are the same rule with the variables "
                           "renamed",
            va="center", color=INK2, fontsize=9.5)
    ax.text(0.02, -0.14, "two different primes annihilate, and the ruler "
...
def rulers():
    ...
    def update(f):
        ax.clear()
        style_axes(ax, (0, 1), (0, 1), equal=False)
        ax.set_title("two nestings that never share a scale", color=INK2,
                     fontsize=12)
        ...
        for y, factor, colour, label in [(0.66, 2, VIOLET,
            ...
            for k in range(6):
                ...
                w /= factor
        if settled >= 24:
            ax.text(0.5, 0.06, "nothing is left when the two are asked "
                               "to agree",
                    ha="center", color=RED, fontsize=11.5)
        tag.set_text("three is harmless in the base-two world, and two is "
                     "harmless in the base-three world")
        return ()
...
def solidify():
    ...
    def update(f):
        ...
        ax.set_title("a doughnut, solidified", color=INK2, fontsize=11.5)
        style_axes(axb, (0, 1), (0, 1), equal=False)
        axb.set_title("the holes that fall out", color=INK2, fontsize=11.5)
...
def homology_bars():
    ...
    fig.subplots_adjust(left=0.02, right=0.99, top=0.8, bottom=0.24,
                        wspace=0.12)
    fig.suptitle("the holes of five shapes, computed over the whole numbers",
    ...
    for ax, name in zip(axes, order):
        ...
        ax.text(1.0, -0.58, "degree", ha="center", va="center", color=MUTED,
                fontsize=9)
    fig.text(0.5, 0.055, "teal: a plain hole.     brown, with its order "
                         "written in: a hole you must go round that many "
                         "times to close.",
