...
(Everything here is backed by an open repository: [github.com/muchmirul/conjectures](https://github.com/muchmirul/conjectures/tree/main/jacobian-conjecture). It contains the code for every animation and a test suite that re-checks every mathematical claim.)
...
Python's `sympy` library works with polynomials exactly, the way you would with a pencil and unlimited patience. The [companion repo](https://github.com/muchmirul/conjectures/tree/main/jacobian-conjecture) wraps it in a few helpers. Here is a real session with the stacked-shear map $H(x,y) = (x + (y+x^2)^2,\; y + x^2)$:
